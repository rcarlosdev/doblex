      ...
      <div class="flex flex-col lg:flex-row justify-between lg:items-center gap-4 border-b border-neutral-100 dark:border-neutral-900 pb-5">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
          <Input 
            ...
            v-model="searchQuery" 
            class="w-full lg:w-72 bg-white dark:bg-neutral-950/60 border-neutral-200 dark:border-neutral-800 text-neutral-800 dark:text-white text-xs placeholder:text-neutral-450 focus-visible:ring-primary focus-visible:border-primary"
          />
          <select 
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="ot in filteredOts" :key="ot.id" class="border-neutral-100 dark:border-neutral-900 hover:bg-neutral-50 dark:hover:bg-white/[0.02]">
                <TableCell class="font-mono font-bold text-primary pl-4 py-4">{{ ot.codigo }}</TableCell>
                <TableCell class="py-4">
                  <div class="flex flex-col">
                    <span class="font-medium text-neutral-900 dark:text-white whitespace-nowrap">{{ ot.descripcion }}</span>
                    <span class="text-[10px] text-neutral-500 mt-0.5 whitespace-nowrap">Inicio: {{ ot.fecha_inicio }}</span>
                  </div>
                </TableCell>
                ...
                <TableCell class="py-4">
                  <div class="flex items-center gap-3">
                    <Progress :model-value="ot.progreso" class="h-1.5 w-20 bg-neutral-100 dark:bg-white/5" />
                ...
                <TableCell class="text-center py-4">
                  <Badge 
                    ...
                    :class="{
                      'bg-emerald-500/10 border-emerald-500/20 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10': ot.estado === 'finalizado',
                      'bg-primary/10 border-primary/20 text-primary hover:bg-primary/10': ot.estado === 'en_progreso',
                      'bg-amber-500/10 border-amber-500/20 text-amber-600 dark:text-amber-400 hover:bg-amber-500/10': ot.estado === 'pendiente',
                    }"
                  >
                ...
                <TableCell class="text-right pr-4 py-4">
                  <div class="inline-flex gap-1">
                    <!-- Administradores y creadores pueden editar / ver detalles -->
                    <Button 
                      v-if="userRole === 'admin' || userRole === 'administrativo'"
                      variant="ghost" 
                      size="icon" 
                    ...
                    <!-- Operativos reportan avances en frentes de obra asignados -->
                    <Button 
                      v-if="userRole === 'operativo' && ot.estado !== 'finalizado'"
    ...
    <Dialog :open="openCreateModal" @close="openCreateModal = false">
      ...
      <form @submit.prevent="createOt" class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="space-y-1.5">
            <label class="text-[10px] font-bold text-neutral-550 dark:text-neutral-400 uppercase tracking-wider">Ubicación</label>
            <Input type="text" v-model="newOt.ubicacion" required placeholder="Frente de Obra - Km 12" class="bg-white dark:bg-neutral-950 border-neutral-200 dark:border-neutral-850 text-neutral-900 dark:text-white focus-visible:ring-primary focus-visible:border-primary" />
          </div>
        </div>

        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-neutral-550 dark:text-neutral-400 uppercase tracking-wider">Descripción de la Obra</label>
          <Input type="text" v-model="newOt.descripcion" required placeholder="Construcción de alcantarillado y cunetas" class="bg-white dark:bg-neutral-950 border-neutral-200 dark:border-neutral-850 text-neutral-900 dark:text-white focus-visible:ring-primary focus-visible:border-primary" />
        </div>
